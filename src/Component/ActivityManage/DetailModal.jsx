                    ...
                    {detail &&
                        <>
                            ...
                            <div className='pt-3'>
                                ...
                                <p>活動種類 : {detail.activityType.name}</p>
                                <p>活動代碼 : {detail.code ? detail.code: '--'}</p>
                                <p>活動地點 : {detail.place  ?  detail.place : '--'} </p>
                                <p>活動時間 :{detail.start_date ? changeDate(detail.start_date) : '無'} ~ {detail.end_date && changeDate(detail.end_date) } </p>
                                <p>權限 : {readPerson(detail.read)}</p>
                                <p>建立者 : {detail.triggerBy.username}</p>
                                <p>建立時間 : {changeDate(detail.createdAt)}</p>
                                <p>更新時間 : {changeDate(detail.activity_update)}</p>
                            </div>
                        </>
                    }
