...
function CreateActivityTypeModal({ show, handle }) {
    const finder = Finder()
    ...
    function createType() {
        if (type.length >= 6) {
            enqueueSnackbar('活動建立失敗! 標籤不可超過六個字', { variant: 'error' })
        } else if (type.length < 2) {
            enqueueSnackbar('活動建立失敗! 標籤不可低於兩個字', { variant: 'error' })
        } else {
            setIsCreate(true)
            finder.post('/activity/type', { name: type }, {
                headers: {
    ...
    function handleDelete() {
        ...
        finder.delete('/activity/type',
            ...
            if (!data) {
                ...
                handleClose()
                console.error("An unknown error occurred");
                enqueueSnackbar('活動刪除失敗! 只有管理員可以刪除活動', { variant: 'error' })
                return;
            }
            ...
            enqueueSnackbar(`活動刪除失敗! ${err.response.data}`, { variant: 'error' })
    ...
    function handleEdit() {
        ...
        finder.patch('/activity/type',
            ...
            if (!data) {
                ...
                handleClose()
                console.error("An unknown error occurred");
                enqueueSnackbar('活動更新失敗! 只有管理員可以更新活動', { variant: 'error' })
                return;
            }
            ...
            enqueueSnackbar(`活動更新失敗! ${err.response.data}`, { variant: 'error' })
            console.log(err.response)
        }).finally(() => setIsEdit(false))
    }
    return (
        ...
        <>
            <Modal centered show={show} onHide={handle} size='md'>
                <Modal.Header closeButton>
                    <Modal.Title>活動種類管理</Modal.Title>
                </Modal.Header>
                <Modal.Body className='px-5'>
                    <Row>
                        <Col md={8}>
                            <Form>
                                <FloatingLabel label="新增活動標籤" >
            ...
            <Modal show={deleteModal} onHide={handleClose} centered>
                <Modal.Header closeButton>
                    <Modal.Title>刪除活動標籤</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    {selectedType &&
                        <p className='pt-3'>
                            請問您確定要刪除「<span className='text-danger'>{selectedType.name}</span>」活動標籤嗎?
                        </p>
                    }
                </Modal.Body>
                <Modal.Footer>
            ...
            <Modal show={editModal} onHide={handleClose} centered>
                <Modal.Header closeButton>
                    <Modal.Title>編輯活動標籤</Modal.Title>
                </Modal.Header>
                <Modal.Body>
